...
async def seed() -> None:
    async with AsyncSessionLocal() as session:
        ...
        me = User(id=1, username="me", display_name="나", bio="오늘의 푸시업 자세를 기록합니다.", is_mock=True)
        coach = User(id=2, username="coach_ai", display_name="AI 코치", bio="자세 피드백을 남기는 코치 계정입니다.", is_mock=True)
        runner = User(id=3, username="daily_push", display_name="데일리 푸시업", bio="매일 한 세트씩.", is_mock=True)
        ...
        session.add_all(
            [
                ...
                Message(sender_id=coach.id, receiver_id=me.id, content="오늘 분석 결과 확인해 보세요."),
